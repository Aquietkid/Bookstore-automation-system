...
router.get('/all/items', async (req, res) => {
  connection.connect(function (err) {
    if (err) throw err;
    connection.query('SELECT id as ID, name AS Name, price AS Price, quantity AS Qty FROM inventory', (error, results) => {
      if (err) throw err;
      else if (results.length == 0) {
        return res.send('No items in inventory! Please add an item to continue!');
      }
